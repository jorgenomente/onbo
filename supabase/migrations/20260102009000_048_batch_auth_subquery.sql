-- Batch update RLS policies to evaluate auth/current_setting once per statement.
...
using (
  ...
  or (
    course_assignments.user_id = (select auth.uid())
    and exists (
      select 1
      from public.location_memberships lm
      where lm.location_id = course_assignments.location_id
...
to authenticated
using (
  group_memberships.user_id = (select auth.uid())
);
...
drop policy if exists "location_invites_delete_admins" on public.location_invites;
...
drop policy if exists "location_invites_select_admins" on public.location_invites;
...
drop policy if exists "location_invites_update_admins" on public.location_invites;
...
drop policy if exists "location_invites_write_admins" on public.location_invites;
...
using (
  ...
  or location_memberships.user_id = (select auth.uid())
);

drop policy if exists "location_memberships_select_own" on public.location_memberships;
