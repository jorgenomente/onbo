-- Consolidate overlapping permissive policies that apply to authenticated (including public).
-- Generated from latest policy definitions in supabase/migrations.
-- Note: consolidated policies target authenticated to avoid public/authenticated overlap.
...
drop policy if exists "location_memberships_select_authenticated" on public.location_memberships;
create policy "location_memberships_select_authenticated"
...
using (
  ...
  or location_memberships.user_id = (select auth.uid())) or
    (user_id = (select auth.uid()))
);
