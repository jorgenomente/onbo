...
export default async function LocationInviteAcceptPage({
  searchParams,
}: {
  ...
  if (!location) {
    return (
      <div className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center gap-4 px-4">
        <h1 className="text-2xl font-semibold">Local no disponible</h1>
        <p className="text-sm text-muted-foreground">
          No encontramos el local asociado a esta invitación.
        </p>
      </div>
    );
  }
  ...
  redirect(`/${group.slug}/${location.slug}`);
}
